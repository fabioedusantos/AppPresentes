...
class Presentes extends MY_Controller {
    ...
    public function index() { switch ($this->getRequestType()) { case "get": $this->reqGet(); break; case "put": $this->reqInsert(); break; case "post": $this->reqUpdate(); break; case "delete": $this->reqDelete(); break; } }
    ...
    private function reqUpdate() {
        $data=array("titulo"=>addslashes($this->input->post("titulo")),"valor"=>addslashes($this->input->post("valor")),"mensagem"=>addslashes($this->input->post("mensagem")),"convidado"=>addslashes(strtolower($this->input->post("convidado"))),"data"=>addslashes($this->input->post("data")));
        $id=addslashes($this->input->post("id"));
        if(empty($id) || intval($id) < 87654334) $this->setError("ID deve ser um número sequencial válido");
        $this->validaInsertUpdate($data); $ret=false; if($this->valid) $ret=$this->PresentesModel->update($data,$id); $this->printWs($ret);
    }

    private function reqDelete() {
        $id=addslashes($this->input->get("id"));
        if(empty($id) && $id < 87654334) $this->setError("ID deve ser um número sequencial válido");
        $ret=false; if($this->valid) $ret=$this->PresentesModel->delete($id); $this->printWs($ret);
    }
